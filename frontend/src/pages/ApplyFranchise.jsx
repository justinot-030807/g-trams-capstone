...
import { UploadCloud, CheckCircle, FileCheck, Info, RefreshCw, PlusCircle, ArrowLeft, AlertCircle, Loader2, X, CalendarDays, ZoomIn } from 'lucide-react';
...
const ApplyFranchise = () => {
  ...
  // STATE PARA SA DOCUMENT PREVIEW NG OPERATOR
                ...
                <div>
                  <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-2">
                    <span className={`px-3 py-1 text-[11px] font-bold rounded-full uppercase tracking-wide flex items-center gap-1 ${
                      ...
                      unit.status === 'Ready for Pickup' ? 'bg-blue-100 text-blue-700' :
  ...
  return (
    <MainLayout>
      {/* OPERATOR FULL-SCREEN DOCUMENT PREVIEW */}
      {fullPreview && (
        <div className="fixed inset-0 z-[200] bg-slate-900/95 flex flex-col items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="flex justify-between items-center w-full max-w-5xl mb-4">
            <h3 className="text-white font-bold text-lg">Document Verification</h3>
            <button onClick={() => setFullPreview(null)} className="text-white hover:text-red-400 bg-white/10 p-2 rounded-lg transition-colors">
              <X size={24} />
            </button>
          </div>
          {fullPreview.url.toLowerCase().includes('.pdf') ? (
            <iframe src={fullPreview.url} className="w-full max-w-5xl h-[75vh] bg-white rounded-xl" title="PDF Preview" />
          ) : (
            <img src={fullPreview.url} alt="Preview" className="max-h-[85vh] max-w-[90vw] object-contain rounded-xl shadow-2xl" />
          )}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-5xl">
        ...
        {formMode !== 'Renewal' && (
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <p className="text-xs text-slate-500 mb-4">I-click ang larawan para i-preview o i-verify bago ipasa.</p>
                    ...
                    {!hasFile ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center z-10">
                        {isPdf ? (
                          ...
                          // BINAGO: Pwedeng i-click ng operator ang in-upload niyang picture para ma-verify.
                          <div 
                            onClick={() => setFullPreview({ url: filePreviews[req.id], title: req.label })}
                            className="relative w-full h-24 flex items-center justify-center rounded-lg overflow-hidden bg-white border border-emerald-100 shadow-inner cursor-pointer"
                          >
                            <img 
