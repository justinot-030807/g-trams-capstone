...
const ApplyFranchise = () => {
  ...
  const disabledClasses = "w-full bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400 outline-none cursor-not-allowed select-none min-h-[44px]";
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
                <div className="sm:col-span-2 lg:col-span-1">
                      ...
                      {POPULAR_MAKES.map((brand) => (
                        <button
                          ...
                          className={`px-2.5 py-1 text-[11px] font-semibold rounded-lg border transition-all cursor-pointer ${
                            formData.make === brand
                              ? 'bg-[#7A1B22] text-white border-[#7A1B22] dark:bg-[#D4AF37] dark:text-slate-950 dark:border-[#D4AF37]'
                  ...
                  <input 
                    ...
                    required 
                    readOnly={formMode === 'Renewal' || formMode === 'Re-apply'} 
                    placeholder="e.g. Honda TMX 125 or enter custom" 
                  />
                </div>
                ...
                <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-2.5">
                  <button 
                    ...
                    onClick={() => {
                      ...
                      if (document.startViewTransition) {
                        document.startViewTransition(cancelFn);
                      } else {
                        cancelFn();
                      }
                    }}
