  ...
  return (
    <MainLayout>
      <header className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Submit TODA Members</h1>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">Upload the official list of your tricycle operators and drivers.</p>
        </div>
      </header>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 sm:p-8 transition-colors">
            ...
            <form onSubmit={handleUpload}>
              <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-8 text-center hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
                <UploadCloud className="mx-auto text-slate-400 dark:text-slate-500 mb-4" size={48} />
                <p className="text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Drag and drop your file here, or click to browse</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-6">Supported formats: PDF, Excel (.xlsx, .csv)</p>
                ...
                <label 
                  htmlFor="file-upload"
                  className="inline-flex items-center justify-center bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 px-6 py-2.5 rounded-xl font-bold text-sm cursor-pointer transition-colors shadow-sm"
                >
                  Browse Files
                </label>

                {file && (
                  <div className="mt-6 flex items-center justify-center gap-2 text-sm font-bold text-[#7A1B22] dark:text-[#D4AF37] bg-[#7A1B22]/10 dark:bg-[#D4AF37]/10 py-2 px-4 rounded-lg inline-flex">
                    <FileText size={16} />
                    {file.name}
                  </div>
                )}
