    ...
    <MainLayout>
      <header className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">Manage operators, roles, and account access.</p>
          </div>
        </div>
                  ...
                  <tr key={user._id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors group ${user.isActive === false ? 'opacity-70 bg-red-50/30 dark:bg-red-950/20' : ''}`}>
                    <td className="p-4 pl-6">
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`font-bold ${user.isActive === false ? 'text-red-900 dark:text-red-300 line-through decoration-red-300' : 'text-slate-900 dark:text-white'}`}>{user.name}</span>
                      </div>
                    </td>

                    <td className="p-4">
                      <p className="text-sm font-bold text-slate-900 dark:text-white">{user.contact}</p>
                      <p className="text-[10px] font-bold text-[#7A1B22] dark:text-[#D4AF37] uppercase tracking-wider bg-[#7A1B22]/10 dark:bg-[#7A1B22]/25 inline-block px-1.5 rounded border border-[#7A1B22]/20 dark:border-[#7A1B22]/40 mt-0.5">
      ...
      {isDetailsModalOpen && selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
          ...
          <div className="relative bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4 text-slate-700 dark:text-slate-300">
              <div className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-100 dark:border-slate-800">
                ...
                <div><p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Barangay Address</p><p className="text-sm font-bold text-slate-900 dark:text-white">{selectedUser.address}</p></div>
              </div>
              <div className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-100 dark:border-slate-800">
                <Phone className="text-slate-400 mt-0.5" size={18} />
                <div><p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Contact Details</p><p className="text-sm font-bold text-slate-900 dark:text-white">{selectedUser.contact}</p></div>
              </div>
              <div className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-100 dark:border-slate-800">
                <Calendar className="text-slate-400 mt-0.5" size={18} />
                <div><p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Date Registered</p><p className="text-sm font-bold text-slate-900 dark:text-white">{new Date(selectedUser.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p></div>
              </div>
            </div>
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60 text-center">
