...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const menu = {
    'admin': [
      ...
      { name: 'Approvals', path: '/franchise-approval', icon: <FileCheck size={16} />, badge: 1 }, // <-- RED BADGE INDICATOR
          ...
          {activeMenu.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* SIDEBAR NOTIFICATION BADGE (RED DOT / NUMBER) */}
                {item.badge && (
                  <span className="flex h-4 min-w-[16px] items-center justify-center rounded-full bg-red-500 px-1 text-[9px] font-black text-white shadow-sm ring-1 ring-white/50 animate-pulse">
                    {item.badge}
                  </span>
                )}
              </button>
